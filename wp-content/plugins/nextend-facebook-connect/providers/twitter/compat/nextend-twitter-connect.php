...
function new_twitter_login_action() {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'unlink') {
        ...
        AND type = \'twitter\'', $user_info->ID));
            set_site_transient($user_info->ID . '_new_twitter_admin_notice', 'Your Twitter profile is successfully unlinked from your account.', 3600);
        }
        ...
        ', $ID));
            ...
            if (!is_user_logged_in()) {
                ...
                if (new_twitter_is_user_connected()) {
                    ...
                    $user_info = wp_get_current_user();
                    set_site_transient($user_info->ID . '_new_twitter_admin_notice', 'Your Twitter profile is successfully linked with your account. Now you can sign in with Twitter easily.', 3600);
                } else {
                    $user_info = wp_get_current_user();
                    set_site_transient($user_info->ID . '_new_twitter_admin_notice', 'This Twitter profile is already linked with other account. Linking process failed!', 3600);
                }
            }
            new_twitter_redirect();
        } else {
...
function new_twitter_plugin_action_links($links, $file) {
    ...
    $settings_link = '<a href="' . esc_url(menu_page_url('nextend-twitter-connect', false)) . '">' . esc_html('Settings') . '</a>';
